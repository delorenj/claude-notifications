...
function sendNotification(notification, pluginName = 'zellij_visual_notifications', options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Escape the JSON properly for shell - replace single quotes with '"'"'
      const escapedPayload = jsonPayload.replace(/'/g, "'\"'\"'");
      try {
        // Broadcast to all plugins listening on this named pipe
        // The plugin will filter messages it cares about
        execSync(`zellij pipe --name notification -- '${escapedPayload}'`, {
