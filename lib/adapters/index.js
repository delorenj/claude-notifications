 ...
 *
 * OpResult:     { changed: boolean, alreadyInstalled?: boolean, reason?: string, diff?: string }
...
/**
 * Read JSON from disk. Returns `{}` when the file is missing or the input
 * parses to something other than a plain object (we only ever upsert hooks
 * into an object-shaped config). Strips a leading BOM so files edited by
 * tools that emit one (Notepad etc.) are still readable. Any parse error
 * bubbles up so the adapter can decide whether to abort or repair.
 */
 ...
 * cannot truncate the target config. Tests supply a fake fs that doesn't
 * implement `renameSync`; in that case we fall back to a direct write.
 */
function writeJson(filePath, obj, { dryRun = false, deps = {} } = {}) {
  ...
  if (typeof fsMod.renameSync === "function") {
    ...
    fsMod.writeFileSync(filePath, serialized);
  }
  return serialized;
}
