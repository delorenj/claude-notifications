...
async function install(ctx) {
  ...
  if (!summary.changed) {
    return { changed: false, alreadyInstalled: true, reason: "up to date" };
  }
...
async function uninstall(ctx) {
  ...
  if (!fsMod.existsSync(target)) {
    return { changed: false, reason: "no config present" };
  }
  let config;
  try {
    config = readJsonSafe(target, deps);
  } catch (err) {
    return { changed: false, reason: `invalid JSON at ${target}: ${err.message}` };
  }
  const summary = mutateConfig(config, "", "uninstall");
  if (!summary.changed) {
    return { changed: false, reason: "nothing to remove" };
  }
  writeJson(target, config, { dryRun, deps });
  return {
    changed: true,
...
async function status(ctx) {
  ...
  const present = events.some(
    (e) =>
      ...
      config.hooks[e].some((x) => x && x.source === "claude-notifications"),
  );
  return {
    installed: present,
    present: true,
